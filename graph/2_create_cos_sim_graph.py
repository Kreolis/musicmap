...
def create_cos_path(file_folder:str):
    path = get_dir_root(file_folder)
    # read in data
    dir_data = path["graph"]
    for key in ["max", "mean"]:
        cos_sim_songs = np.load(os.path.join(dir_data, f"cos_sim_songs_{key}.npy"))
        ...
        nx.write_adjlist(G, os.path.join(dir_data, f"cos_sim_songs_graph_{key}"))
